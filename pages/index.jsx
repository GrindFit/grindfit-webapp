...
export default function MemberHome(){
  ...
  useEffect(()=>{ try{ const raw=localStorage.getItem("grindfit_profile"); setProfile(raw?JSON.parse(raw):null);}catch{} },[]);
        ...
        <section className="grid md:grid-cols-2 lg:grid-cols-3 gap-4">
          {cards.map(c=>(
            <article key={c.title} className="gf-card flex flex-col">
              ...
              <div className="mt-4">
                <Link className="btn-ghost no-underline" href={c.href}>Open {c.title}</Link>
              </div>
            </article>
          ))}
